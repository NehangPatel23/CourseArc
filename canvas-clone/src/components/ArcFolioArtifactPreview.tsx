...
function FileThumb({ file }: { file: ArcFolioFilePreview }) {
  ...
  const fileIcon: IconName = file.kind === "unknown" ? "folder" : "file";
  return (
    <div className="flex h-full flex-col items-center justify-center gap-2 px-4 text-center">
      <Icon name={fileIcon} size={32} className="text-arc-mute" />
...
function CardChrome({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group relative mb-3 h-36 w-full overflow-hidden bg-arc-ivory text-left ring-1 ring-arc-ink/10 transition hover:ring-arc-copper/40 hover:shadow-lift"
    >
      {children}
      <span className="pointer-events-none absolute inset-x-0 bottom-0 flex items-center justify-center gap-1 bg-gradient-to-t from-arc-ink/55 to-transparent py-2 text-[11px] font-semibold text-arc-cream opacity-0 transition group-hover:opacity-100">
...
export function ArcFolioCardPreview({
    ...
    return (
      <CardChrome onClick={onOpenPreview}>
        <div className="flex h-full items-center gap-4 px-4">
          <div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center bg-arc-gold/20 text-arc-ink">
            <Icon name="help" size={16} className="mb-0.5" />
      ...
      <CardChrome onClick={onOpenPreview}>
        <div className="flex h-full flex-col justify-between p-3">
          <div className="flex items-start gap-2">
            <Icon name="chat" size={16} className="mt-0.5 shrink-0 text-arc-sage" />
  ...
  if (preview.kind === "external-url") {
    const typeIcon: IconName =
      preview.externalType === "github" || preview.externalType === "website"
        ? "globe"
        : "link";
    return (
      <CardChrome onClick={onOpenPreview}>
        <div className="flex h-full flex-col">
          <div className="flex items-center gap-2 border-b border-arc-ink/10 bg-arc-paper/70 px-3 py-1.5">
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-2 px-4 text-center">
            <Icon name={typeIcon} className="h-7 w-7 text-arc-copper/80" size={28} />
  ...
  return (
    <CardChrome onClick={onOpenPreview}>
      <div className="flex h-full flex-col items-center justify-center gap-2 px-4 text-center">
        <Icon name="file" size={28} className="text-arc-mute" />
          ...
          <ul className="divide-y divide-gray-100 rounded-xl border border-gray-200">
            {preview.attempts.map((a) => (
              ...
              >
                ...
                <span className="font-semibold tabular-nums text-arc-copper">
                  {a.scoreLabel}
                </span>
              </li>
            ))}
      ...
      <div className="flex min-h-[420px] flex-col">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-200 bg-gray-50 px-4 py-2">
          ...
          <a
            ...
            className="inline-flex shrink-0 items-center gap-1 text-xs font-semibold text-arc-copper hover:underline"
          >
            Open in new tab
        ...
        {canEmbed ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center">
            <Icon name="globe" size={40} className="text-arc-mute" />
...
export function ArcFolioPreviewModal({
  ...
  return (
    <CanvasModal title={entry.title} onClose={onClose} size="preview">
      ...
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-arc-ink/10 bg-arc-ivory px-5 py-3">
        <p className="text-xs text-gray-500">
